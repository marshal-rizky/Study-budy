...
describe("AnthropicClient", () => {
  it("maps the request into Anthropic's shape (tools, tool_use, tool_result, thinking)", async () => {
    ...
    // Extended thinking defaults on, with plenty of token budget here.
    expect(params.thinking).toEqual({ type: "enabled", budget_tokens: 1024 });
  });

  it("skips thinking when maxTokens can't fit the budget, or when disabled", async () => {
    const create = vi.fn(async (_params: unknown) => ({
      content: [],
      stop_reason: "end_turn",
    ...
    const tightBudget = new AnthropicClient({
      apiKey: "k",
      model: "claude-fable-5",
      client: { messages: { create: create as never } },
    });
    await tightBudget.createMessage({ ...BASE_REQUEST, maxTokens: 512 });
